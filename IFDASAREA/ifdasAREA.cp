...
CS_INT  m_giCALL_TIMES;
CS_INT  m_giCALL_TOTAL;
...
CS_CHAR m_gszCUST_TYPE[6];
CS_CHAR m_gszCUST_TYPE_DESC[20];
CS_CHAR m_gszCUST_MANAGER_ID[10];
CS_CHAR m_gszCUST_MANAGER_NAME[60];
...
CS_CHAR	m_gszLONG_LIMIT[30];
CS_CHAR	m_gszTIME_CIRCLE_SCALE[5];
CS_CHAR	m_gszTOTAL_CIRCLE_SCALE[5];
	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			if(strcmp(m_gszPERIODS,"WEEK")==0)
			{
				...
				SELECT CONVERT(VARCHAR,DATEADD(day,-7,:m_gszBEGIN_DATE),112) INTO :m_gszPRE_BEGIN_DATE;
				EXEC SQL at connectionsybase 
				SELECT CONVERT(VARCHAR,DATEADD(day,-7,:m_gszEND_DATE),112) INTO :m_gszPRE_END_DATE;
				strcpy(g_tablename,"STAT_AREA_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				EXEC SQL at connectionsybase 
				SELECT CONVERT(VARCHAR,DATEADD(month,-1,:m_gszBEGIN_DATE),112) INTO :m_gszPRE_BEGIN_DATE;
				EXEC SQL at connectionsybase 
				SELECT CONVERT(VARCHAR,DATEADD(day,-1,:m_gszBEGIN_DATE),112) INTO :m_gszPRE_END_DATE;
				strcpy(g_tablename,"STAT_AREA_MONTH");
			}
			else if(strcmp(m_gszPERIODS,"DATE")==0)
			{
				EXEC SQL at connectionsybase 
				SELECT CONVERT(VARCHAR,DATEADD(day,-1,:m_gszBEGIN_DATE),112) INTO :m_gszPRE_BEGIN_DATE;
				EXEC SQL at connectionsybase 
				SELECT CONVERT(VARCHAR,DATEADD(day,-1,:m_gszEND_DATE),112) INTO :m_gszPRE_END_DATE;
				strcpy(g_tablename,"STAT_AREA_DATE");
			}
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='AREA' AND ISNULL(STATUS,'0') IN ('1','2','e','E') 
	ORDER BY STATUS DESC,ISNULL(PRIORITY,0) DESC,BEGIN_DATE ASC;
...
void drawdata()
{
	sprintf(g_sql,"INSERT INTO %s(ACC_NBR,AREA_TYPE,CALL_TIMES,CALL_TOTAL,BEGIN_DATE,END_DATE,CREATE_DATE) "
	"SELECT NUM as ACC_NBR,AREA_TYPE,CALL_OUT_TIMES,CALL_OUT_TOTAL,TJ_BEGIN_DATE,TJ_END_DATE,GETDATE() FROM JKT_AREA_CALL "
	"WHERE AREA_TYPE='%s' AND TJ_BEGIN_DATE='%s' AND TJ_END_DATE='%s' AND CALL_TOTAL>=%d",
	g_tablename,m_gszBEGIN_DATE,m_gszEND_DATE,m_gszDATA_KIND,m_gszBEGIN_DATE,m_gszEND_DATE,m_giCALL_TOTAL_LIMIT);
	
	EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;
	EXEC SQL at connectionsybase COMMIT WORK;
			
	EXEC SQL at connectionsybase DELETE FROM JKT_AREA_CALL
	WHERE AREA_TYPE=:m_gszDATA_KIND AND TJ_BEGIN_DATE=:m_gszBEGIN_DATE AND TJ_END_DATE=:m_gszEND_DATE;
	EXEC SQL at connectionsybase COMMIT WORK;
}

void patchdata()
{
	sprintf(g_sql,"SELECT ACC_NBR,CALL_TIMES,CALL_TOTAL FROM %s WHERE AREA_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s' AND ISNULL(STATUS,'0') IN ('0','E')",
	g_tablename,m_gszDATA_KIND,m_gszBEGIN_DATE,m_gszEND_DATE);
	EXEC SQL PREPARE select_stmt FROM :g_sql;

	EXEC SQL DECLARE area CURSOR FOR select_stmt;
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH area INTO :m_gszACC_NBR, :m_giCALL_TIMES, :m_giCALL_TOTAL;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
	...
	EXEC SQL at connectionsybase CLOSE area;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR area;
...
void updatedata()
{
	...
	"BLACK_AREA='%s',LONG_LIMIT='%s',TIME_CIRCLE_SCALE='%s',TOTAL_CIRCLE_SCALE='%s',STATUS='2' WHERE ACC_NBR='%s'",g_tablename,
	...
	m_gszLINE_BOX_NBR,m_gszCROSS_BOX_NBR,m_gszBLACK_AREA,m_gszLONG_LIMIT,m_gszTIME_CIRCLE_SCALE,m_gszTOTAL_CIRCLE_SCALE,m_gszACC_NBR);
	EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;
...
int GetDataInfo()
{
	...
	ISNULL(LTRIM(RTRIM(CUST_MANAGER_NAME)),''),ISNULL(LTRIM(RTRIM(LONG_LIMIT)),'')  
	...
	:m_gszCUST_MANAGER_NAME, :m_gszLONG_LIMIT FROM SERV_INFO 
	...
	sprintf(g_sql,"SELECT CONVERT(VARCHAR,CONVERT(INT,(%d-CALL_TIMES)*100/CONVERT(NUMERIC(10,2),CALL_TIMES)))+'%' "
	"FROM %s WHERE ACC_NBR='%s' AND AREA_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s'",m_giCALL_TIMES,
	g_tablename,m_gszACC_NBR,m_gszDATA_KIND,m_gszPRE_BEGIN_DATE,m_gszPRE_END_DATE);
	
	EXEC SQL PREPARE select_stmt FROM :g_sql;
	
	EXEC SQL at connectionsybase EXECUTE select_stmt INTO :m_gszTIME_CIRCLE_SCALE;
		
	sprintf(g_sql,"SELECT CONVERT(VARCHAR,CONVERT(INT,(%d-CALL_TOTAL)*100/CONVERT(NUMERIC(10,2),CALL_TOTAL)))+'%' "
	"FROM %s WHERE ACC_NBR='%s' AND AREA_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s'",m_giCALL_TOTAL,
	g_tablename,m_gszACC_NBR,m_gszDATA_KIND,m_gszPRE_BEGIN_DATE,m_gszPRE_END_DATE);
	
	EXEC SQL PREPARE select_stmt FROM :g_sql;
	
	EXEC SQL at connectionsybase EXECUTE select_stmt INTO :m_gszTOTAL_CIRCLE_SCALE;
	...
	void InitialDataInfo()
	{
		...
		m_gszBLACK_AREA[0]       ='\0';
		m_gszLONG_LIMIT[0]       ='\0';
		m_gszTIME_CIRCLE_SCALE[0]='\0';
		m_gszTOTAL_CIRCLE_SCALE[0]='\0';
	}
