	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			if(strcmp(m_gszPERIODS,"WEEK")==0)
			{
				...
				strcpy(g_tablename,"STAT_LDD_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				...
				strcpy(g_tablename,"STAT_LDD_MONTH");
			}
			else if(strcmp(m_gszPERIODS,"DATE")==0)
			{
				...
				strcpy(g_tablename,"STAT_LDD_DATE");
			}
			
			
			m_giCALL_TIME_LIMIT=0;
			if(strcmp(m_gszDATA_KIND,"DIS_3")==0)
			{
				m_giCALL_TOTAL_LIMIT=30;
			}
			else if(strcmp(m_gszDATA_KIND,"DIS_4")==0)
			{
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='LDD' AND DATA_KIND IN ('DIS_3','DIS_4') 
...
void drawdata()
{
	sprintf(g_sql,"INSERT INTO %s(ACC_NBR,DIS_TYPE,CALL_TIMES,CALL_TOTAL,BEGIN_DATE,END_DATE,CREATE_DATE) "
	"SELECT NUM as ACC_NBR,DIS_TYPE,CALL_OUT_TIMES,CALL_OUT_TOTAL,TJ_BEGIN_DATE,TJ_END_DATE,GETDATE() FROM JKT_LONG_DIS "
	"WHERE DIS_TYPE='%s' AND TJ_BEGIN_DATE='%s' AND TJ_END_DATE='%s' AND CALL_OUT_TIMES>=%d AND CALL_OUT_TOTAL>=%d",
...
void patchdata()
{
	sprintf(g_sql,"SELECT ACC_NBR,CALL_TIMES,CALL_TOTAL FROM %s WHERE DIS_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s' AND ISNULL(STATUS,'0') IN ('0','E')",
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH ldd INTO :m_gszACC_NBR, :m_giCALL_TIMES, :m_giCALL_TOTAL;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
...
void updatedata()
{
	...
	"BLACK_AREA='%s',TIME_CIRCLE_SCALE='%s',TOTAL_CIRCLE_SCALE='%s',STATUS='2' WHERE ACC_NBR='%s'",g_tablename,
	...
	m_gszLINE_BOX_NBR,m_gszCROSS_BOX_NBR,m_gszBLACK_AREA,m_gszTIME_CIRCLE_SCALE,m_gszTOTAL_CIRCLE_SCALE,m_gszACC_NBR);
	EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;
...
int GetDataInfo()
{
	...
	"FROM %s WHERE ACC_NBR='%s' AND DIS_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s'",m_giCALL_TIMES,
	...
	"FROM %s WHERE ACC_NBR='%s' AND DIS_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s'",m_giCALL_TOTAL,
	g_tablename,m_gszACC_NBR,m_gszDATA_KIND,m_gszPRE_BEGIN_DATE,m_gszPRE_END_DATE);
