...
int main()
{
	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			...
			{
				strcpy(g_tablename,"STAT_ZERO_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				strcpy(g_tablename,"STAT_ZERO_MONTH");
			}
			if(m_gszSSTATUS[0]=='1')
			{
				write_log("INFO","开始进行ZERO的处理");
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='ACTION' AND DATA_KIND='ZERO' AND ISNULL(STATUS,'0') IN ('1','2','e','E') ORDER BY STATUS DESC,ISNULL(PRIORITY,0) DESC,BEGIN_DATE ASC;
...
void drawdata()
{
	sprintf(g_sql,"INSERT INTO %s(ACC_NBR,BEGIN_DATE,END_DATE,CREATE_DATE) "
	"SELECT NUM,TJ_BEGIN_DATE,TJ_END_DATE,GETDATE() FROM JKT_USER_ACTION "
	"WHERE TJ_BEGIN_DATE = CONVERT(DATETIME,'%s',112) AND TJ_END_DATE = CONVERT(DATETIME,'%s',112) "
	"AND ACTION_TYPE='ZERO' AND ISNULL(STATUS,'0')='0'",
	...
	EXEC SQL at connectionsybase DELETE FROM JKT_USER_ACTION 
	WHERE ACTION_TYPE='ZERO' AND TJ_BEGIN_DATE = CONVERT(DATETIME,:m_gszBEGIN_DATE,112) AND TJ_END_DATE = CONVERT(DATETIME,:m_gszEND_DATE,112);
	EXEC SQL at connectionsybase COMMIT WORK;
}
...
void patchdata()
{
	...
	EXEC SQL DECLARE zero CURSOR FOR select_stmt;
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH zero INTO :m_gszACC_NBR;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
	...
	EXEC SQL at connectionsybase CLOSE zero;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR zero;
...
void setzeroreason()
{
	EXEC SQL BEGIN DECLARE SECTION;
		...
		CS_CHAR m_szZERO_REASON_TYPE[10];
		CS_CHAR m_szZERO_REASON[200];
	EXEC SQL END DECLARE SECTION;
	
	EXEC SQL DECLARE zeroreason CURSOR FOR 
		SELECT ACC_NBR,ZERO_REASON_TYPE,ZERO_REASON FROM ACTION_ZERO_LIST;
		
	EXEC SQL OPEN zeroreason;
	for (int i=0;;i++)
	{
		EXEC SQL at connectionsybase FETCH zeroreason INTO 
		:m_szACC_NBR,:m_szZERO_REASON_TYPE,:m_szZERO_REASON;
		
		if (sqlca.sqlcode == 100)
				break;
		
		sprintf(g_sql,"UPDATE %s SET REASON_TYPE='%s',REASON='%s' "
		"WHERE ACC_NBR='%s' AND BEGIN_DATE='%s' AND END_DATE='%s'",
		g_tablename,m_szZERO_REASON_TYPE,m_szZERO_REASON,m_szACC_NBR,m_gszBEGIN_DATE,m_gszEND_DATE);
		EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;

		EXEC SQL at connectionsybase COMMIT WORK;
	}
	EXEC SQL at connectionsybase CLOSE zeroreason;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR zeroreason;
	printf("setZeroReason Complete \n");
	write_log("INFO","setZeroReason Complete");
}
