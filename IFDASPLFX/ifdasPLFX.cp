...
int main()
{
	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			...
			{
				strcpy(g_tablename,"STAT_PLFX_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				strcpy(g_tablename,"STAT_PLFX_MONTH");
			}
			else if(strcmp(m_gszPERIODS,"DATE")==0)
			{
				strcpy(g_tablename,"STAT_PLFX_DATE");
			}
			if(m_gszSSTATUS[0]=='1')
			{
				write_log("INFO","开始进行PLFX的处理");
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='YXFL' AND DATA_KIND='PLFX' AND ISNULL(STATUS,'0') IN ('1','2','e','E') ORDER BY STATUS DESC,ISNULL(PRIORITY,0) DESC,BEGIN_DATE ASC;
...
void drawdata()
{
	sprintf(g_sql,"INSERT INTO %s(ACC_NBR,ISP_TYPE,FROM_AREA,TO_AREA,CALL_TIMES,CALL_TOTAL,BEGIN_DATE,END_DATE,CREATE_DATE) "
	"SELECT ACC_NBR,ISP_TYPE,FROM_AREA,TO_AREA,CALL_TIMES,CALL_TOTAL,TJ_BEGIN_DATE,TJ_END_DATE,GETDATE() FROM JKT_PLFX "
	"WHERE TJ_BEGIN_DATE='%s' AND TJ_END_DATE='%s'",
	g_tablename,m_gszBEGIN_DATE,m_gszEND_DATE);

	EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;
	EXEC SQL at connectionsybase COMMIT WORK;
			
	EXEC SQL at connectionsybase DELETE FROM JKT_PLFX
		WHERE ISP_TYPE=:m_gszDATA_KIND AND TJ_BEGIN_DATE=:m_gszBEGIN_DATE AND TJ_END_DATE=:m_gszEND_DATE;
	EXEC SQL at connectionsybase COMMIT WORK;
}

void patchdata()
{
	sprintf(g_sql,"SELECT ACC_NBR FROM %s WHERE BEGIN_DATE='%s' AND END_DATE='%s' AND ISNULL(STATUS,'0') IN ('0','E')",
	g_tablename,m_gszBEGIN_DATE,m_gszEND_DATE);
	EXEC SQL PREPARE select_stmt FROM :g_sql;

	EXEC SQL DECLARE plfx CURSOR FOR select_stmt;
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH plfx INTO :m_gszACC_NBR;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
	...
	EXEC SQL at connectionsybase CLOSE plfx;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR plfx;
...
{
	EXEC SQL BEGIN DECLARE SECTION;
		CS_CHAR m_szCOUNTER_FLAG[5];
		CS_CHAR m_szAFFIRM_FLAG[5];
		CS_CHAR m_szILLEGAL_NOTES[200];
		CS_CHAR m_szACC_NBR[30];
	EXEC SQL END DECLARE SECTION;
	
	EXEC SQL DECLARE illigal CURSOR FOR 
		SELECT ACC_NBR,COUNTER_FLAG,AFFIRM_FLAG,ILLEGAL_NOTES FROM YXFL_FFZX;
		
	EXEC SQL OPEN illigal;
	for (;;)
	{
		EXEC SQL at connectionsybase FETCH illigal INTO 
		:m_szACC_NBR,:m_szCOUNTER_FLAG,:m_szAFFIRM_FLAG,:m_szILLEGAL_NOTES;
		
		if (sqlca.sqlcode == 100)
				break;
		
		sprintf(g_sql,"UPDATE %s SET COUNTER_FLAG='%s',AFFIRM_FLAG='%s',ILLEGAL_NOTES='%s' "
		"WHERE ACC_NBR='%s' AND BEGIN_DATE='%s' AND END_DATE='%s'",
		g_tablename,m_szCOUNTER_FLAG,m_szAFFIRM_FLAG,m_szILLEGAL_NOTES,m_szACC_NBR,m_gszBEGIN_DATE,m_gszEND_DATE);
		EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;

		EXEC SQL at connectionsybase COMMIT WORK;
	}
	EXEC SQL at connectionsybase CLOSE illigal;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR illigal;
	printf("setIlligalInfo Complete \n");
	write_log("INFO","setIlligalInfo Complete");
