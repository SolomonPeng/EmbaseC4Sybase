...
int main()
{
	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			...
			{
				strcpy(g_tablename,"STAT_OTIP_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				strcpy(g_tablename,"STAT_OTIP_MONTH");
			}
			if(m_gszSSTATUS[0]=='1')
			{
				write_log("INFO","开始进行OTIP的处理");
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='OTIP' AND ISNULL(STATUS,'0') IN ('1','2','e','E') ORDER BY STATUS DESC,ISNULL(PRIORITY,0) DESC,BEGIN_DATE ASC;
...
void drawdata()
{
	...
	"SELECT ACC_NBR,ISP_TYPE,CALL_TIMES,CALL_TOTAL,TJ_BEGIN_DATE,TJ_END_DATE,GETDATE() FROM JKT_OTHER_IP "
...
void patchdata()
{
	...
	EXEC SQL DECLARE otip CURSOR FOR select_stmt;
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH otip INTO :m_gszACC_NBR;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
	...
	EXEC SQL at connectionsybase CLOSE otip;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR otip;
