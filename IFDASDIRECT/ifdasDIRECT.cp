...
int main()
{
	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			...
			{
				strcpy(g_tablename,"STAT_DIRECT_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				strcpy(g_tablename,"STAT_DIRECT_MONTH");
			}
			m_giCALL_TOTAL_LIMIT=40;
			if(m_gszSSTATUS[0]=='1')
			{
				write_log("INFO","开始进行DIRECT的处理");
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='DIRECT' AND ISNULL(STATUS,'0') IN ('1','2','e','E') ORDER BY STATUS DESC,ISNULL(PRIORITY,0) DESC,BEGIN_DATE ASC;
...
void drawdata()
{
	sprintf(g_sql,"INSERT INTO %s(ACC_NBR,CALL_NBR,DIRECT_TYPE,CALL_TIMES,CALL_TOTAL,BEGIN_DATE,END_DATE,CREATE_DATE) "
	"SELECT OLD_CALL_NBR,CALL_NBRM,'%s',SUM(CALL_TIME),COUNT(*),'%s','%s',GETDATE() FROM JKT_DIRECT "
	"WHERE CALL_DATE>=CONVERT(DATETIME,'%s',112) AND CALL_DATE<DATEADD(DAY,1,CONVERT(DATETIME,'%s',112)) AND DIRECT_TYPE = '%s' "
	"GROUP BY OLD_CALL_NBR,CALL_NBR HAVING COUNT(*)>%d",
	g_tablename,m_gszDATA_KIND,m_gszBEGIN_DATE,m_gszEND_DATE,m_gszBEGIN_DATE,m_gszEND_DATE,m_gszDATA_KIND,m_giCALL_TOTAL_LIMIT);

	EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;
	EXEC SQL at connectionsybase COMMIT WORK;
			
	EXEC SQL at connectionsybase DELETE FROM JKT_DIRECT
	WHERE DIRECT_TYPE=:m_gszDATA_KIND AND CALL_DATE>=CONVERT(DATETIME,:m_gszBEGIN_DATE,112) AND CALL_DATE<DATEADD(DAY,1,CONVERT(DATETIME,:m_gszEND_DATE,112));
	EXEC SQL at connectionsybase COMMIT WORK;
}

void patchdata()
{
	sprintf(g_sql,"SELECT ACC_NBR FROM %s WHERE DIRECT_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s' AND ISNULL(STATUS,'0') IN ('0','E')",
	g_tablename,m_gszDATA_KIND,m_gszBEGIN_DATE,m_gszEND_DATE);
	EXEC SQL PREPARE select_stmt FROM :g_sql;

	EXEC SQL DECLARE direct CURSOR FOR select_stmt;
	

	EXEC SQL OPEN direct;
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH direct INTO :m_gszACC_NBR;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
	...
	EXEC SQL at connectionsybase CLOSE direct;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR direct;
