...
void MatchOrgan(CS_INT,ORGAN_STRUCT*);
void GetDataInfo();
...
{
	EXEC SQL 
	SELECT SERV_NBR,ISNULL(LTRIM(RTRIM(PROD_ID)),''),EXCHANGE_ID,
	...
	INTO	:m_gszSERV_NBR, :m_gszPROD_ID, :m_giEXCHANGE_ID, :m_gszCUST_NAME, 
	:m_gszCUST_ADDRESS, :m_gszCERT_TYPE_ID,:m_gszCERT_ID, :m_gszCUST_CLASS_DL_ID, 
	:m_gszCUST_CLASS_XL_ID,:m_gszRELA_MAN, :m_gszRELA_PHONE, :m_gszCUST_NBR, 
	:m_gszCUST_PROP_ID,:m_gszSYSTEM_NBR, :m_gszLINE_BOX_NBR, :m_gszCROSS_BOX_NBR,
	:m_gszBLACK_AREA FROM SERV_INFO 
	...
	MatchOrgan(m_giEXCHANGE_ID,&g_org);
	...
	void MatchOrgan(CS_INT iEXCHANGE_ID,ORGAN_STRUCT* organstruct)
	{
	...
	char* trim(char* rel,const char* s,int n)   
  {   
    const char* se = 0;   
    assert( s );   
    assert( strlen( s ) <= n && strlen( s ) != 0 );   
    se = s + strlen( s ) - 1;   
    for( ; isspace( *s ); ++s );   
    for( ; isspace( *se ); --se );   
    strncpy( rel , s , se - s + 1 );   
    return rel;   
  }
