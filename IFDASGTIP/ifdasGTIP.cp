...
int main()
{
	...
	while(g_runflag)
	{
		if(getstatistics())
		{
			...
			{
				strcpy(g_tablename,"STAT_GTIP_WEEK");
			}
			else if(strcmp(m_gszPERIODS,"MONTH")==0)
			{
				strcpy(g_tablename,"STAT_GTIP_MONTH");
			}
			if(m_gszSSTATUS[0]=='1')
			{
				write_log("INFO","开始进行GTIP的处理");
...
int getstatistics()
{
	...
	FROM STATISTICS_INFO WHERE STAT_TYPE='GTIP' AND ISNULL(STATUS,'0') IN ('1','2','e','E') ORDER BY STATUS DESC,ISNULL(PRIORITY,0) DESC,BEGIN_DATE ASC;
...
void drawdata()
{
	sprintf(m_gszDATA_KINDG,"G%s",m_gszDATA_KIND);
	sprintf(g_sql,"INSERT INTO %s(ACC_NBR,IP_NUM,CALL_TIMES,CALL_TOTAL,BEGIN_DATE,END_DATE,CREATE_DATE) "
	"SELECT ACC_NBR,%s,CALL_TIMES,CALL_TOTAL,TJ_BEGIN_DATE,TJ_END_DATE,GETDATE() FROM JKT_OTHER_IP "
	"WHERE ISP_TYPE='%s' AND TJ_BEGIN_DATE='%s' AND TJ_END_DATE='%s'",
	g_tablename,m_gszDATA_KIND,m_gszDATA_KINDG,m_gszBEGIN_DATE,m_gszEND_DATE);

	EXEC SQL at connectionsybase EXECUTE IMMEDIATE :g_sql;
	EXEC SQL at connectionsybase COMMIT WORK;
			
	EXEC SQL at connectionsybase DELETE FROM JKT_OTHER_IP
		WHERE ISP_TYPE=:m_gszDATA_KINDG AND TJ_BEGIN_DATE=:m_gszBEGIN_DATE AND TJ_END_DATE=:m_gszEND_DATE;
	EXEC SQL at connectionsybase COMMIT WORK;
}

void patchdata()
{
	sprintf(g_sql,"SELECT ACC_NBR FROM %s WHERE ISP_TYPE='%s' AND BEGIN_DATE='%s' AND END_DATE='%s' AND ISNULL(STATUS,'0') IN ('0','E')",
	g_tablename,m_gszDATA_KIND,m_gszBEGIN_DATE,m_gszEND_DATE);
	EXEC SQL PREPARE select_stmt FROM :g_sql;

	EXEC SQL DECLARE gtip CURSOR FOR select_stmt;
	...
	for (int i=0;;i++)
	{
		...
		InitialDataInfo();
		EXEC SQL at connectionsybase FETCH gtip INTO :m_gszACC_NBR;
		g_totalrows++;
		if (sqlca.sqlcode == 100)
		{
			break;
	...
	EXEC SQL at connectionsybase CLOSE gtip;
	EXEC SQL at connectionsybase DEALLOCATE CURSOR gtip;
